---
title: Test implementation of fullGP, NNGP, SGV in nimble with large N
author: Mark Risser
date: 22 March 2019
...
We'll use some simulated data for now, with $N=1000$ and $M=1225$:
...
# save(samplesList, predSamplesList, timeList, file = "~/CASCADE_Projects/bayes_nsgp/test_implementation_bigN.RData")
...
load("~/CASCADE_Projects/bayes_nsgp/test_implementation_bigN.RData")
...
Compare compute times (minutes) for the MCMC:

```{r, echo = FALSE}
data.frame(
  Method = c("fullGP","NNGP","SGV"),
  MCMC_time = c(timeList$fullGP[3]/60, timeList$NNGP[3]/60, timeList$SGV[3]/60)
)
```

Compare compute times (minutes) for the prediction:

```{r, echo = FALSE}
data.frame(
  Method = c("fullGP","NNGP","SGV"),
  Predict_time = c(timeList$fullGP.pred[3]/60, timeList$NNGP.pred[3]/60, timeList$SGV.pred[3]/60)
)
```
