...
prt <- proc.time()
samples_fullGP <- runMCMC(Cmcmc, niter = nIter, nburnin = nBurnin)
...
Cmcmc <- compileNimble(Rmcmc, project = Rmodel)
prt <- proc.time()
samples_NNGP <- runMCMC(Cmcmc, niter = nIter, nburnin = nBurnin)
...
Cmcmc <- compileNimble(Rmcmc, project = Rmodel)
prt <- proc.time()
samples_SGV <- runMCMC(Cmcmc, niter = nIter, nburnin = nBurnin)
...
grid.arrange(
    ggplot(data.frame(x = locs[,1], y = locs[,2], data = z), aes(x = x, y = y, color = data)) + labs(x = NULL, y = NULL) +
      geom_point() + scale_color_gradientn(colors = brewer.pal(9, "Spectral")) + coord_fixed(ratio = 1),
    ggplot(data.frame(x = locs[,1], y = locs[,2], obsPred = colMeans(predSamplesList$SGV$obs)), aes(x = x, y = y, color = obsPred)) + 
      labs(x = NULL, y = NULL) +
      geom_point() + scale_color_gradientn(colors = brewer.pal(9, "Spectral")) + coord_fixed(ratio = 1),
