---
title: Test implementation of fullGP, NNGP, SGV in nimble -- locally isotropic
author: Mark Risser
date: 28 March 2019
...
We'll use some simulated data for now. To test the isotropic case, use 3-dimensional coordinates.
...
predCoords <- as.matrix(expand.grid(seq(0,1,length = 6),seq(0,1,length = 6),seq(0,1,length = 6)))
...
```{r }
# FullGP 
dist_fullGP <- nsDist(locs, isotropic = TRUE)
constants_fullGP <- list( dist1_sq = dist_fullGP$dist1_sq, dist2_sq = dist_fullGP$dist2_sq, 
                          dist12 = dist_fullGP$dist12, nu = 0.5, N = N, Sigma_HP1 = 2, d = 3 )
...
dist_SGV <- nsDist3d(locs[SGV_setup$ord,], nID_SGV, isotropic = TRUE)
...
Rmodel_fullGP <- nsgpModel( likelihood = "fullGP", Sigma_model = "constantIso", 
...
```{r, eval=FALSE}
# Defaults: tau_model = "constant", sigma_model = "constant", mu_model = "constant"
Rmodel_NNGP <- nsgpModel( likelihood = "NNGP", Sigma_model = "constantIso", 
                          constants = constants_NNGP, z = z )
...
```{r, eval=FALSE}
# Defaults: tau_model = "constant", sigma_model = "constant", mu_model = "constant"
Rmodel_SGV <- nsgpModel( likelihood = "SGV", Sigma_model = "constantIso", 
                         constants = constants_SGV, z = z_unord[SGV_setup$ord] )
