...
for(r in 1:6){
  conf$addSampler(target = c(paste0("alpha[", r, "]"),
                             paste0("Sigma_coef1[", r, "]")), type = "RW_block")
}
conf$getSamplers()
# Build
...
# Run
prt <- proc.time()
samples <- runMCMC(Cmcmc, niter = 6000, nburnin = 0)
time_mcmc <- proc.time() - prt
save(samples, time_mcmc, time_build, file = "app3_alt1.RData")

par(ask=TRUE)
for(h in 1:ncol(samples)) plot(samples[-(1:500),h], type = "l", main = colnames(samples)[h])
par(ask=FALSE)
...
prt <- proc.time()
pred_NNGP <- nsgpPredict(model = Rmodel, samples = samples[seq(from=105,to=200,by=5),],
                         coords.predict = predCoords, constants = PREDconstants)
time_NNGP_pred <- proc.time() - prt
save(pred_NNGP, time_NNGP_pred, file = "app3_rv20_NNGP_pred_newModel.RData")
