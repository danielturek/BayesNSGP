...
# ...and constants
constants_PacScher <- list( nu = 2, N = N, Sigma_knot_coords = knot_coords,
...
prt <- proc.time()
pred_PacScher <- nsgpPredict(model = Rmodel, samples = samples_PacScher[seq(10,10000,10),], 
                             coords.predict = predCoords, 
                             Sigma_cross_dist_obs = constants_PacScher$Sigma_cross_dist,
                             Sigma_cross_dist_pred = mahalanobis.dist(predCoords, knot_coords, diag(2)) )
time_PacScher_pred <- proc.time() - prt

save(samples_PacScher, time_PacScher, pred_PacScher, time_PacScher_pred, file = "PacScher.RData")

# grid.arrange(
#   ggplot(predCoords, aes(x = longitude, y = latitude, fill = apply(pred_PacScher$pred, 2, mean))) + geom_tile() +
#     scale_fill_gradientn(colors = viridis_pal(option = "viridis")(9), name = "Mean") + coord_fixed(ratio = 1.1),
#   ggplot() +
#     geom_tile(data = predCoords, mapping = aes(x = longitude, y = latitude, fill = apply(pred_PacScher$pred, 2, sd)))+
#     scale_fill_gradientn(colors = viridis_pal(option = "viridis")(9), name = "SD", trans = "log", breaks = c(0.01,0.03,0.1,0.3)) + coord_fixed(ratio = 1.1) +
#     geom_point(data = as.data.frame(coords), mapping = aes(x = Longitude, y = Latitude), color = "white", size = 0.5),
#   ncol = 2
# )
...
```{r, eval = FALSE}
constants_RisserCalder <- list( nu = 0.5, N = N,
...
Rmodel <- nsgpModel(likelihood = "fullGP", constants = constants_RisserCalder, 
                    coords = coords, z = z, 
...
save(samples_RisserCalder, time_RisserCalder, pred_RisserCalder, time_RisserCalder_pred, file = "RisserCalder.RData")

# grid.arrange(
#   ggplot(predCoords, aes(x = longitude, y = latitude, fill = apply(pred_RisserCalder$pred, 2, mean))) + geom_tile() +
#     scale_fill_gradientn(colors = viridis_pal(option = "viridis")(9), name = "Mean") + coord_fixed(ratio = 1.1),
#   ggplot() +
#     geom_tile(data = predCoords, mapping = aes(x = longitude, y = latitude, fill = apply(pred_RisserCalder$pred, 2, sd)))+
#     scale_fill_gradientn(colors = viridis_pal(option = "viridis")(9), name = "SD", trans = "log", breaks = c(0.01,0.03,0.1,0.3)) + coord_fixed(ratio = 1.1) +
#     geom_point(data = as.data.frame(coords), mapping = aes(x = Longitude, y = Latitude), color = "white", size = 0.5),
#   ncol = 2
# )
...
data.frame(
  ...
  MCMC_time = round(c(time_PacScher[3]/60, time_RisserCalder[3]/60),1),
  Prediction_time = round(c(time_PacScher_pred[3]/60, time_RisserCalder_pred[3]/60),1)
)
```
...
data.frame(
  ...
  PosteriorMean = round(c(mean(samples_PacScher[,"beta"]), mean(samples_PacScher[,"alpha"]),
                    mean(samples_PacScher[,"delta"])), 3),
  PosteriorSD = round(c(sd(samples_PacScher[,"beta"]), sd(samples_PacScher[,"alpha"]),
                   sd(samples_PacScher[,"delta"])), 3)
)
...
```{r, echo = FALSE, fig.height=6, fig.width=10, warning = FALSE}
grid.arrange(
  plot_spat(coords, round(colMeans(log(samples_PacScher[,1:217])),1), pt.size = 1.25, shp = 16, brk_rd = 1,
            ttle.txt = "Posterior mean: log(Sigma11)", 
            col.pal = viridis_pal(option = "viridis")(9)),
  plot_spat(coords, round(colMeans(log(samples_PacScher[,435:651])),1), pt.size = 1.25, shp = 16, brk_rd = 1,
            ttle.txt = "Posterior mean: log(Sigma22)", 
            col.pal = viridis_pal(option = "viridis")(9)),
  plot_spat(coords, colMeans(samples_PacScher[,218:434]), pt.size = 1.25, shp = 16, brk_rd = 3,
            ttle.txt = "Posterior mean: Sigma12", col.lim = c(-0.05, 0.05),
...
```{r, echo = FALSE}
# Look at ellipses
Sigma_postMed <- matrix(apply(samples_PacScher[,1:651], 2, median), ncol = 3, byrow = FALSE)
