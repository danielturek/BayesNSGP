...
save(samples, time_mcmc, time_build, file = "app3_rv20_SGV_samples_time.RData")
...
prt <- proc.time()
pred_SGV <- nsgpPredict(model = Rmodel, samples = samples[seq(from=10002,to=20000,by=2),], 
                         coords.predict = predCoords,
                         PX_sigma = Xmat2_pred, PX_Sigma = Xmat2_pred, PX_mu = Xmat1_pred)
time_SGV_pred <- proc.time() - prt
save(pred_SGV, time_SGV_pred, file = "app3_rv20_SGV_pred.RData")
